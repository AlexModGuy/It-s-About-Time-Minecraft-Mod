package itsabouttime.client.model.prehistoric.technical;
...
public class MowzieModelBaseIAT
  extends ModelBase
{
  ...
  public void faceTarget(MowzieModelRenderer2 box, int divider, float f3, float f4)
  {
    box.rotateAngleY += f3 / 57.295776F / divider;
    box.rotateAngleX += f4 / 57.295776F / divider;
  }
  
  public void walk(MowzieModelRenderer2 box, float speed, float degree, boolean invert, float offset, float weight, float f, float f1)
  {
    int inverted = 1;
    if (invert) {
      inverted = -1;
  ...
  public void flap(MowzieModelRenderer2 box, float speed, float degree, boolean invert, float offset, float f, float f1)
  {
    int inverted = 1;
    if (invert) {
      inverted = -1;
  ...
  public void bob(MowzieModelRenderer2 box, float speed, float degree, boolean bounce, float f, float f1)
  {
  ...
  public void tailSwing(MowzieModelRenderer2[] boxes, float speed, float degree, double rootOffset, float frame)
  {
  ...
  public void chainWave(MowzieModelRenderer2[] boxes, float speed, float degree, double rootOffset, float f, float f1)
  {
    int numberOfSegments = boxes.length;
    float offset = (float)(rootOffset * 3.141592653589793D / (2 * numberOfSegments));
    for (int i = 0; i < numberOfSegments; i++) {
