...
import net.minecraft.world.gen.layer.IntCache;public class WorldChunkManagerDenovian extends WorldChunkManager
{
 ...
 */
 ...
 private List biomesToSpawnIn;@SuppressWarnings("unchecked")
protected WorldChunkManagerDenovian()
{
 this.biomeCache = new BiomeCache(this);
 this.biomesToSpawnIn = new ArrayList<BiomeGenBase>();
 this.biomesToSpawnIn.add(IATbiomes.DenovianBeach);
 this.biomesToSpawnIn.add(IATbiomes.DenovianHills);
 this.biomesToSpawnIn.add(IATbiomes.DenovianRiver);
 this.biomesToSpawnIn.add(IATbiomes.DenovianSea);
 this.biomesToSpawnIn.add(IATbiomes.DenovianSwamp);
 this.biomesToSpawnIn.add(IATbiomes.DenovianForest);
}public WorldChunkManagerDenovian(long seed, WorldType worldtype)
{
this();
GenLayer[] agenlayer = GenLayerDenovian.makeTheWorld(seed);
this.genBiomes = agenlayer[0];
this.biomeIndexLayer = agenlayer[1];
}public WorldChunkManagerDenovian(World world)
{
...
{
 IntCache.resetIntCache();if (par1ArrayOfFloat == null || par1ArrayOfFloat.length < par4 * par5)
 {
 par1ArrayOfFloat = new float[par4 * par5];
 }int[] aint = this.biomeIndexLayer.getInts(par2, par3, par4, par5);for (int i1 = 0; i1 < par4 * par5; ++i1)
 {
 try
 {
 float f = (float)BiomeGenBase.getBiome(aint[i1]).getIntRainfall() / 65536.0F;if (f > 1.0F)
 {
 f = 1.0F;
...
public boolean areBiomesViable(int par1, int par2, int par3, List par4List)
{
 ...
 int l1 = j1 - l + 1;
 int i2 = k1 - i1 + 1;
 int[] aint = this.genBiomes.getInts(l, i1, l1, i2);try
 {
...
public ChunkPosition findBiomePosition(int p_150795_1_, int p_150795_2_, int p_150795_3_, List p_150795_4_, Random p_150795_5_)
{
 ...
 int l2 = l + k2 % l1 << 2;
 int i3 = i1 + k2 / l1 << 2;
 BiomeGenBase biomegenbase = BiomeGenBase.getBiome(aint[k2]);if (p_150795_4_.contains(biomegenbase) && (chunkposition == null || p_150795_5_.nextInt(j2 + 1) == 0))
 {
